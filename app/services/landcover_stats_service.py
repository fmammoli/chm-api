...
def _decode_pmtiles_png_tile(tile_bytes: bytes, compression: pm_reader.Compression) -> np.ndarray:
    if compression == pm_reader.Compression.GZIP:
        tile_bytes = gzip.decompress(tile_bytes)
    elif compression not in {pm_reader.Compression.NONE, pm_reader.Compression.GZIP}:
